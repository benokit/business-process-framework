...
| `getElementsOfKindHierarchy` | Like `getElementsOfKind`, but walks a slash-separated class hierarchy. Takes `{ "_prefix": <expr>, "_hierarchy": <expr> }`: splits `_hierarchy` by `/` and returns elements of `_prefix + "a"`, `_prefix + "a/b"`, …, `_prefix + "a/b/c"` concatenated in that order. Returns `{ items: [] }` when `_hierarchy` is absent or empty. Example: `{ "getElementsOfKindHierarchy": { "_prefix": "\\my-kind/", "_hierarchy": "#.entityHierarchy" } }` |
...
| `execute` | Evaluates a lambdaJSON expression against the full context; the result is used as a pipeline (single node or array) to execute inline. Use `{ "$literal": <pipeline> }` to pass a static pipeline. To invoke a keyword with a runtime value, use a plain-object expression: `{ "execute": { "someKeyword": "#.input.value" } }` — lambdaJSON evaluates the object, producing `{ "someKeyword": <runtime-value> }` as the node to execute. |
...
| `publish` | Publishes a message to a channel: `{ "publish": { "channel": "<id>", "envelope": {...} } }` — registered by the `messaging` package; see [`publish` pipeline keyword](../framework/messaging/README.md#publish-pipeline-keyword) |
